...
export function runFullCoordination(year: number = 2026, quarter: QuarterPeriod = 'Q2'): AgentCoordinationState {
  // Step 1: CEO sets business targets
  const ceoResult = runCEOAgent(year, quarter);

  // Step 2: HR Director maps targets to department goals
  const hrResult = runHRDirectorAgent(ceoResult.targets);

  // Step 3: Department Managers break goals into individual plans
  const deptResult = runDeptManagerAgent(hrResult.goals);

  // Step 4: Performance Coach monitors and flags risks
  const coachResult = runPerformanceCoachAgent(deptResult.plans);

  // Step 5: Finance calculates costs and salary projections
  const financeResult = runFinanceAgent(hrResult.goals);

  // Step 6: Generate financial statements
  const financials = generateAllFinancials();
  ...
  const departmentDetails = generateDepartmentDetails(
